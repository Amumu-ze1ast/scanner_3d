...
class MoveIn_auto_StraightLineState(EventState):
    '''
    Moves the Gen3 arm end-effector in a straight line from the robot's current pose to end position.
    ...
    def __init__(self, end_x, end_y, end_z, 
                 num_waypoints=5, roll=0.0, pitch=np.pi/2, yaw=0.0, duration=10.0):
        super(MoveIn_auto_StraightLineState, self).__init__(
            outcomes=['reached', 'failed'],
            input_keys=['target_position']
        )
    ...
    def on_enter(self, userdata):
        ...
        # Get start position from userdata (previous state's output)
        if 'target_position' in userdata:
            self._start = np.array(userdata.target_position)
            Logger.loginfo(f'Using start position from previous state: {self._start}')
        else:
            Logger.logerr('No target_position in userdata!')
            self._failed = True
            return
