...
class DetectObjectSimpleState(EventState):
    '''
    ...
    <= detected                  Object detected, coordinates logged
    <= failed                    Detection failed or timeout
    '''
    
    def __init__(self, base_frame="base_link", min_area=800, timeout=5.0):
        super(DetectObjectSimpleState, self).__init__(outcomes=['detected', 'failed'])
        
        self._base_frame = base_frame
        self._min_area = min_area
        self._timeout = timeout
        self._hsv_ranges = [[17, 0, 0, 180, 255, 255]]  # Default HSV range
